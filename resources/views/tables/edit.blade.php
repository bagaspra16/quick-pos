...
<div class="card rounded-lg shadow-md p-6">
    <h2 class="text-2xl font-semibold mb-6">Edit Meja</h2>
    ...
    <form action="{{ route('tables.update', $table) }}" method="POST">
        ...
        <div class="mb-4">
            ...
            <input type="text" id="number" name="number" value="{{ old('number', $table->number) }}" required
        ...
        <div class="mb-4">
            ...
            <input type="number" id="capacity" name="capacity" value="{{ old('capacity', $table->capacity) }}" min="1" required
                   class="w-full bg-gray-700 border border-gray-600 rounded-md px-4 py-2 focus:outline-none focus:border-blue-500">
            @error('capacity')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
